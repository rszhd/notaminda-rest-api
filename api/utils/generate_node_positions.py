    ...
    while iterations < max_iterations:
        ...
        for i in range(len(new_nodes)):
            for j in range(len(all_nodes)):
                if new_nodes[i].get('id') != all_nodes[j].get('id') and detect_collision(new_nodes[i], all_nodes[j]):
...
def detect_collision(node1, node2):
    return (
        node1['position']['x'] < node2['position']['x'] + node2['width'] and
        node1['position']['x'] + node1['width'] > node2['position']['x'] and
        node1['position']['y'] < node2['position']['y'] + node2['height'] and
