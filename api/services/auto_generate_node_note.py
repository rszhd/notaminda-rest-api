...
class NodeNoteGenerator:
    @classmethod
    def generate(cls, node: Node, instruction: Optional[str] = None, ai_key: Optional[str] = None, ai_model: Optional[str] = None) -> Dict[str, Any]:
        try:
            ...
            thread = threading.Thread(target=cls._run_chat_stream, args=(node.id, messages, ai_key, ai_model))
            thread.start()
    ...
    def _get_nodes(node: Node) -> List[Dict[str, Any]]:
        nodes = Node.objects.filter(mind_map=node.mind_map).values('id', 'title', 'parent', 'flow_data')
        return [
            {
                'id': node['id'],
                'label': json.loads(node['flow_data'])['data']['label'] if node['flow_data'] else node['title'],
                'parent': node['parent']
            }
            for node in nodes
        ]
    ...
    @staticmethod
    def _create_message(node: Node, nodes: List[Dict[str, Any]], instruction: Optional[str]) -> str:
    ...
    @classmethod
    def _run_chat_stream(cls, node_id: int, messages: List[Dict[str, str]], ai_key: Optional[str], ai_model: Optional[str]):
        try:
            OpenaiUtil.chat_stream(
                messages=messages,
                on_stream=lambda result: cls._send_stream_result(node_id, result),
                api_key=ai_key, model=ai_model
            )
        except Exception as e:
    ...
    @classmethod
    def _send_socket_request(cls, action: str, dataset_id: int, data: Optional[Dict[str, Any]] = None):
        try:
            payload = {
                "isSuccess": True,
                "action": action,
